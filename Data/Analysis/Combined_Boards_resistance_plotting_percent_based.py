            ...
            for j, filename in enumerate(lvm_files):
                with open(os.path.join(new_file_address, filename), 'r') as file:
                    ...
                    try:
                        ...
                        if resistance < MAX_VALID_RESISTANCE: # Adds resistance if valid
...
for i in range(len(board_names)):
    plt.plot(impact_numbers_percents[i], resistance_numbers_list[i],  marker='o', linestyle='', label=board_names[i])
