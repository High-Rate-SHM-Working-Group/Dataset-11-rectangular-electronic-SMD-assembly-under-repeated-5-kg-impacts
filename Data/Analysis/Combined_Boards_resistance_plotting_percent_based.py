...
for i in range(3):
    plt.plot(fit_line_x_values[i+4], fit_line_y_values[i+4], label=board_names[i+4])
for i in range(3):
    plt.plot(impact_numbers_percents[i+4], resistance_numbers_percents[i+4],  marker='o', linestyle='', label=board_names[i+4])
...
for i in range(len(board_names)):
    plt.figure(figsize=(10,6))
    plt.plot(fit_line_x_values[i], fit_line_y_values[i], label=board_names[i])
    plt.plot(impact_numbers_percents[i], resistance_numbers_percents[i],  marker='o', linestyle='', label=board_names[i])
    plt.legend()
    plt.xlabel('Impact Number')
    plt.ylabel('Resistance Change (Ohms)')
